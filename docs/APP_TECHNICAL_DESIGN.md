...
- UI: AppBar、mode ごとの content、`_ModeSelector`、固定 `AudioPlayerBar`。content は `TranscriptList`、`_QuestionContent`、footer 付き `TranscriptList`、`_ExplanationContent`。選択肢・正解または解説が未収録なら専用案内を表示する。切り替え中の全画面overlayとPlayer内Spinnerは表示せず、操作だけを一時的に無効化する。
...
| `AudioPlayerBar` | `ConsumerWidget` / 前後問題callback、`interactionEnabled` | audio State、settings Controller | seek、play、speed save、前後問題、3再生mode切り替え | 切り替えlock中は全操作無効、Spinner非表示。Widget testあり |
...
sequenceDiagram
    ...
    and 音源準備
        ...
        Audio->>Service: 単一問題loop設定 + loadAsset + speed復元
    end
    alt load成功
        Audio-->>Page: ready
...
| `test/features/player/presentation/audio_player_bar_test.dart` | `AudioPlayerBar`の前後問題操作と3再生mode | Widget | 先頭・中央・末尾・両側なしと切り替えlock中をpumpし、左右・modeボタンをtap | 境界表示、mode巡回、Spinner非表示、Slider / speed / play無効化 | 成功 |
...
| `test/features/practice/presentation/n2_practice_integration_test.dart` | N2練習教材の一覧・詳細・無感切り替え・Test制約 | Widget | q02音源loadを保留してRoute更新し、完了後に連続再生、失敗時にBackを実行 | load前の目標本文、Spinnerなし、準備後再生、失敗時の目標維持とSnackBar、無効Test card | 成功 |
...
対象範囲はSplash起動遷移とlifecycle、App smoke、本文同期の境界、同梱教材validation、Learning / Test Repositoryの主要write-read、無感問題切り替え、Player操作lock、3再生mode、stop / load競合である。未テスト範囲はBottom Navigation stack、Settings Controller、単一問題loopの実音声連続再生、detail自動scroll、Favorites / Settings、Native Splashの実機表示である。
